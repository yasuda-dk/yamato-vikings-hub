  ...
  loop
    ...
    loop
      if participant_item->>'kind' = 'member' and not exists (
        ...
        raise exception 'Only Going active members can be added to draft teams';
      end if;
      ...
      if participant_item->>'kind' = 'guest' and not exists (
        ...
        raise exception 'Only event guests can be added to draft teams';
      end if;
...
begin
  ...
  if p_action = 'remove-participant' then
    if p_participant_kind not in ('member', 'guest') or p_participant_id is null then
    ...
    if exists (
      ...
      raise exception 'Unlock this participant before removing';
    end if;
