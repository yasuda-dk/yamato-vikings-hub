create or replace function public.list_admin_season_events(
  target_team_id uuid,
  target_year integer
)
returns table (
...
as $$
  ...
  where e.team_id = target_team_id
    ...
    and public.is_admin(target_team_id)
    and e.event_date >= make_date(target_year, 1, 1)
    and e.event_date < make_date(target_year + 1, 1, 1)
  group by e.id, my_attendance.rsvp_status
  order by e.event_date asc, e.start_time asc;
$$;

grant execute on function public.list_admin_season_events(uuid, integer) to authenticated;
