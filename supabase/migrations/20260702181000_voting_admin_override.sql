  ...
  if target_event.status = 'Completed' then
    with visible_awards as (
      ...
      where award.event_id = target_event_id
        and (
          award.is_admin_override = true
          or not exists (
            select 1
            from public.event_awards override_award
            where override_award.event_id = award.event_id
              and override_award.award_type = award.award_type
              and override_award.is_admin_override = true
          )
        )
    )
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'is_winner', true,
          'is_admin_override', award.is_admin_override
        )
        order by award.award_type, award.is_admin_override desc, coalesce(m.first_name, guest.first_name)
      ),
      '[]'::jsonb
    ) into final_results
    from visible_awards award
...
create or replace function public.override_event_award(
  target_event_id uuid,
  p_award_type public.vote_type,
...
begin
  ...
  if target_event.status <> 'Completed' then
    raise exception 'Awards can only be overridden after voting is completed';
  end if;
  ...
  select count(*)::integer into candidate_vote_count
  from public.votes v
  where v.event_id = target_event_id
    and v.vote_type = p_award_type
    and (
      (p_candidate_member_id is not null and v.candidate_member_id = p_candidate_member_id)
      or (p_candidate_event_guest_id is not null and v.candidate_event_guest_id = p_candidate_event_guest_id)
    );

  delete from public.event_awards
  where event_id = target_event_id
    and award_type = p_award_type
    and is_admin_override = true;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'candidate_kind', case when p_candidate_member_id is not null then 'member' else 'guest' end,
      'candidate_id', coalesce(p_candidate_member_id, p_candidate_event_guest_id),
...
grant execute on function public.override_event_award(uuid, public.vote_type, uuid, uuid) to authenticated;
