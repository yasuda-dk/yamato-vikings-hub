import type { ActualStatus, EventParticipant } from './events';
import type { AgeGroup, Position } from './member-options';

export type TeamGenerationParticipant = Pick<EventParticipant, 'kind' | 'id' | 'first_name' | 'football_level' | 'primary_position' | 'secondary_position'> & {
  age_group: AgeGroup;
...
export type TeamGenerationResult = {
  ...
  eligibleParticipants: TeamGenerationParticipant[];
};
...
export function generateTeams(input: TeamGenerationInput): TeamGenerationResult {
  const eligibleParticipants = getEligibleParticipants(input.participants);
  const targetSizes = getTargetSizes(eligibleParticipants.length, input.teamCount);
  const attempts = Math.max(input.attempts ?? 80, 1);
  let bestTeams: TeamGenerationParticipant[][] | null = null;
  let bestScore = Number.POSITIVE_INFINITY;
  let bestBreakdown: ScoreBreakdown | null = null;

  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const rng = createSeededRandom(`${input.seed}:${attempt}`);
    const candidate = createCandidate(eligibleParticipants, targetSizes, input.teamCount, rng, input.lockedAssignments ?? {});
    const improved = improveWithSwaps(candidate, input.lockedAssignments ?? {});
  ...
  const finalTeams = bestTeams ?? Array.from({ length: input.teamCount }, () => []);
  const finalBreakdown = bestBreakdown ?? scoreTeamBuckets(finalTeams).breakdown;

  return {
    teams: finalTeams.map((participants, index) => ({
      name: teamNames[index],
      participants,
      summary: summarizeTeam(participants),
    })),
    score: bestScore,
...
export function getEligibleParticipants(participants: TeamGenerationParticipant[]) {
  return participants.filter((participant) => participant.actual_status === 'Attended' && participant.membership_status !== 'Inactive');
}

export function summarizeTeam(participants: TeamGenerationParticipant[]): TeamSummary {
...
export function scoreTeams(teams: GeneratedTeam[]) {
  return scoreTeamBuckets(teams.map((team) => team.participants));
}
...
function createCandidate(
  ...
  const unlocked = shuffle(
    ...
    rng,
  ).sort((left, right) => right.football_level - left.football_level || rng() - 0.5);
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < current.length; rightIndex += 1) {
        for (let leftPlayerIndex = 0; leftPlayerIndex < current[leftIndex].length; leftPlayerIndex += 1) {
          for (let rightPlayerIndex = 0; rightPlayerIndex < current[rightIndex].length; rightPlayerIndex += 1) {
            const leftParticipant = current[leftIndex][leftPlayerIndex];
            const rightParticipant = current[rightIndex][rightPlayerIndex];
            if (lockedAssignments[getParticipantKey(leftParticipant)] !== undefined || lockedAssignments[getParticipantKey(rightParticipant)] !== undefined) continue;

            const swapped = current.map((team) => [...team]);
            swapped[leftIndex][leftPlayerIndex] = rightParticipant;
            swapped[rightIndex][rightPlayerIndex] = leftParticipant;
...
function getParticipantKey(participant: TeamGenerationParticipant) {
